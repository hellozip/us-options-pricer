...
function parseOptionContract(raw) {
  ...
  let match = compact.match(/^([A-Z][A-Z0-9.-]{0,9})(\d{6})([CP])(\d{8})$/);
  if (match) {
    return {
      symbol: match[1],
      expiry: dateFromYYMMDD(match[2]),
  ...
  if (!symbolMatch || !dateMatch || !typeMatch) {
    throw new Error("无法识别期权格式，请使用 OCC 代码或类似 AAPL 2026-01-16 200 C 的格式");
  }
...
function yearsToExpiry(expiry) {
  ...
  const diffMs = expiryUtc - Date.now();
  return Math.max(diffMs / (365 * 24 * 60 * 60 * 1000), 0);
}
...
function scenarioRow(label, S, params, contractSize) {
  ...
  const contractValue = american * contractSize;
  return {
    html: `
      <tr>
        <td>${label}</td>
...
function calculate() {
  ...
  els.modelMeta.textContent = `${type === "call" ? "Call" : "Put"}，剩余 ${Math.max(T * 365, 0).toFixed(1)} 天，波动率 ${pct(sigma)}，二叉树 ${Math.round(steps)} 步`;

  const intrinsic = intrinsicValue(S, K, type);
  const timeValue = Math.max(base.american - intrinsic, 0);
  let note = `内在价值 ${money(intrinsic)}，美式模型时间价值约 ${money(timeValue)}。`;
  if (type === "call" && q > 0.005) {
    note += " 对看涨期权，股息率越高，美式提前行权价值越可能出现。";
  } else if (type === "put" && intrinsic > 0) {
    note += " 深度实值 Put 对提前行权更敏感，美式价可能高于欧式价。";
  } else {
    note += " 多数非分红 Call 的美式价会非常接近欧式价。";
  }
...
function setQuoteMetrics(data) {
  ...
  if (Number.isFinite(price.regularMarketPrice)) {
    els.spotInput.value = price.regularMarketPrice.toFixed(2);
  }
  if (Number.isFinite(price.postMarketPrice)) {
    els.afterHoursInput.value = price.postMarketPrice.toFixed(2);
  }
  if (!dividendTouched && Number.isFinite(div.estimatedYield)) {
    els.dividendInput.value = (div.estimatedYield * 100).toFixed(2);
  }
...
async function fetchQuote() {
  ...
  try {
    ...
    if (!response.ok || !data.ok) {
      throw new Error(data.error || "行情接口返回失败");
    }
    quoteData = data;
...
function attachEvents() {
  els.parseBtn.addEventListener("click", () => {
    try {
      const contract = parseOptionContract(els.contractInput.value);
